...
subtitle: Extracting Text Data From Files
author: "Jason Freels"
...
- In a text analysis, a common first step is extracting data from a volume of documents that may be stored in multiple different file formats  

- For example, you might need to extract text from documents where one or more have been saved with the following file format types:
...
- Many text analysis tutorials can be found that discuss how to analyze a data set that's  already been extracted from a document and cleaned up -- but this isn't reality

- Therefore, this lecture walks through the process of extracting raw, unstructured text data from the different types of documents listed above.  

- In doing this we'll encounter several Python libraries and/or 3^rd^ party tools that have been developed for this purpose -- we'll demo each of these tools on various data sets

- It's important to mention up front that several text analysis ***frameworks*** exist that provide the analyst with potentially "one-stop" resource to perform text analysis

- While this sounds great, the analyst recognizes (often too late) that the data extracted by these frameworks is stored in an object type that only works with functions/methods that are part of that framework

- Therefore, a problem that's often encountered is when the chosen framework doesn't have an important capability that the analyst wants

- To avoid this problem I don't discuss framework, but instead assume that we'll extract the text data into a baseline form that allows us to work with most any framework or function we want
...
Throughout this document you'll be asked to download and install several python libraries.  While this is a simple task, let's take the extra step by using the `pacman` package.  The `pacman` package allows us to install **and/or** load packages on the fly.  To use `pacman`, you first need to install it on your machine by using the code in the chunk below.
...
In the following sections I demonstrate how to extract text data from various types of file formats. Click on each tab to learn more

# Extracting text from `txt` files

## Overview

- Many different file types can be treated as simple text files, for example
...
- These files can be viewed and/or edited using a basic text editor (such as notepad) similar to we might handle

- Thus, it's important to know how to extract text from these types of files and understand that in some cases it may be preferred to work with text data that is stored in this format

- In this section I'll demonstrate the processes to extract text data stored in `.txt` files and `.html` files
...
We'll need the Natural Language Toolkit or `nltk`
...
In order to install NLTK run the following commands in your terminal (or the conda prompt).

- Run `pip install nltk` at the conda prompt to install the `nltk` library to your Python working environment

- Run `python` the conda prompt to ensure that you are sending commands to python and not to Anaconda (or miniconda)

    + After running this code your prompt symbol should change to `>>>`
    + If this doesn't occur check your spelling making sure that `python` is all lower case 

- Run `import nltk` at the Python prompt to install the `nltk` library

- Run `nltk.download("all")` at the Python prompt to download the massive amount of tokenizers, chunkers, other algorithms, and all of the corpora that are external to the `nltk` library

- Run `exit()` at the Python prompt to exit Python and return to the conda prompt
...
```{python}
url = "https://raw.githubusercontent.com/AFIT-R/oper655_fa2019/master/README.md"

# Allows for arbitrary requests while keeping track of connection pools
http = urllib3.PoolManager()

# perform a GET request to get the content from the URL
response = http.request('GET', url)

# extract the text from the content stored in the GET response
...
[type(sents), type(words), type(lines)]
...
```{python}
opn = open(file = r'../resources/files/meeker_genmax_email.txt',mode = 'r')
